...
class DatabaseSessionManager:
    ...
    def __init__(self, app: Optional[Flask] = None, session_timeout: int = 1800):
        ...
        if app is not None and app.config.get("SQLALCHEMY_DATABASE_URI"):
            ...
            logger.warning("No valid database configuration found - using in-memory session storage only")
        ...
        if self.with_database:
            try:
                ...
                if self.app is not None:
                    ...
                    logger.warning("Flask app context not available - using in-memory only")
