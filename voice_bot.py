...
async def handle_language_level(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text(
        ...
        "questions, or topics that interest you. You can also send voice messages if you prefer."
    )
...
async def handle_voice_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Get language level from session for speech recognition language hint
    ...
    try:
        ...
        # Download voice file
        voice_file = await speech_recognizer.download_voice_file(voice.file_path, TELEGRAM_TOKEN)
        
        if not voice_file:
            await update.message.reply_text(
                "I had trouble downloading your voice message. Please try again or send a text message."
            )
            return
        
        # Transcribe voice message
        await context.bot.send_chat_action(chat_id=user_id, action="typing")
        transcribed_text = await speech_recognizer.transcribe_voice(voice_file)
        ...
        # Add to session
        session_manager.add_message_to_session(user_id, "user", transcribed_text)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle user text messages and continue the discussion."""
    user_id = update.effective_user.id
